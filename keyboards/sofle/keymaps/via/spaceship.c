...
static void render_quartile(int cursor_location,
    const char* space_row,
    const char* mask_row,
    const char* ship_row,
    char wpm) {
    char render_row[128];
    ...
    oled_set_cursor(0, cursor_location);

    for(i = 0; i < 128; i++) {
        render_row[i] = pgm_read_byte(space_row + i + state);

        if (i >= wpm/4) {
            render_row[i] = (render_row[i] & pgm_read_byte(mask_row + i - wpm / 4)) | pgm_read_byte(ship_row + i - wpm / 4);
        }
    }
...
static void render_space(void) {
    ...
    render_quartile(0, space_row_1, mask_row_1, ship_row_1, wpm);
    render_quartile(1, space_row_2, mask_row_2, ship_row_2, wpm);
    render_quartile(2, space_row_3, mask_row_3, ship_row_3, wpm);
    render_quartile(3, space_row_4, mask_row_4, ship_row_4, wpm);
